...
# Rebuild and redeploy Willful Waste Bot
# Use this when you've changed bot code/Dockerfile
# Time: ~30-45 seconds
...
echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  Rebuild & Redeploy Willful Waste Bot                     ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
if [ ! -f "scripts/k3d-up.sh" ]; then
    ...
    echo "Example: cd kubernetes && ./scripts/rebuild-bot.sh"
    exit 1
fi
...
if docker build -t "$IMAGE_NAME" -f "$ROOT_DIR/shared/$SERVICE_DIR/Dockerfile" "$ROOT_DIR/shared/$SERVICE_DIR"; then
    echo -e "${GREEN}✓ Image built successfully: $IMAGE_NAME${NC}"
else
    echo -e "${RED}Error: Failed to build bot image${NC}"
    exit 1
fi
...
if k3d image import "$IMAGE_NAME" -c "$CLUSTER_NAME"; then
    ...
    echo -e "${RED}Error: Failed to import bot image${NC}"
    exit 1
fi

echo ""
echo -e "${GREEN}Restarting willful-waste-bot deployment...${NC}"
kubectl rollout restart deployment/willful-waste-bot -n apollo-dash0-demo

echo -e "${GREEN}Waiting for rollout...${NC}"
kubectl rollout status deployment/willful-waste-bot -n apollo-dash0-demo --timeout=120s > /dev/null 2>&1 || true
...
echo -e "  View logs:         ${BLUE}kubectl logs -f deployment/willful-waste-bot -n apollo-dash0-demo${NC}"
