...
# Rebuild and redeploy Products subgraph (Python)
# Use this when you've changed products subgraph code/Dockerfile
# Time: ~30-45 seconds
...
echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  Rebuild & Redeploy Products Subgraph (Python)            ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
if [ ! -f "scripts/k3d-up.sh" ]; then
    ...
    echo "Example: cd kubernetes && ./scripts/rebuild-products.sh"
    exit 1
fi
...
if docker build -t "$IMAGE_NAME" -f "$ROOT_DIR/shared/subgraphs/$SUBGRAPH_DIR/Dockerfile" "$CONTEXT_DIR"; then
    ...
    echo -e "${RED}Error: Failed to build products image${NC}"
    exit 1
fi
...
if k3d image import "$IMAGE_NAME" -c "$CLUSTER_NAME"; then
    ...
    echo -e "${RED}Error: Failed to import products image${NC}"
    exit 1
fi

echo ""
echo -e "${GREEN}Restarting products deployment...${NC}"
kubectl rollout restart deployment/products -n apollo-dash0-demo

echo -e "${GREEN}Waiting for rollout...${NC}"
kubectl rollout status deployment/products -n apollo-dash0-demo --timeout=120s > /dev/null 2>&1 || true
...
echo -e "  View logs:         ${BLUE}kubectl logs -f deployment/products -n apollo-dash0-demo${NC}"
echo -e "  Pod status:        ${BLUE}kubectl get pods -n apollo-dash0-demo | grep products${NC}"
echo ""
