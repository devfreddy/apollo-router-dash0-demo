...
echo -e "${GREEN}Updating router-config ConfigMap from kubernetes/helm-values/router-values.yaml...${NC}"
kubectl create configmap router-config \
    --from-file=router-values.yaml=kubernetes/helm-values/router-values.yaml \
